...
export class Vector2 {
  ...
  public polarTranslateBy(by: number, length: number): this {
    const angle = cycleNumber(this.angle + by, Math.PI * 2);
  ...
  public rotateByFrom(by: number, from: PointLike): this {
    ...
    const angle = cycleNumber(a + by, Math.PI * 2);
  ...
  static angleIsInProximity(a: number, b: number, tolerance: number): boolean {
    const d1 = differenceClockwise(a, b);
    const d2 = differenceCounterclockwise(a, b);
  ...
  static getAngleBetween2Points(a: PointLike, b: PointLike): number {
    ...
    const b1 = differenceClockwise(a1, a2);
    const b2 = differenceCounterclockwise(a1, a2);
